...
tree=read_tree("C:\\Users\\Joe Receveur\\Documents\\MSU data\\Aquatic Pig\\AquaticPigTree.nwk")
...
sample_data(physeq)$DecompStage = factor(sample_data(physeq)$DecompStage, levels = c("SubmergedFresh","EarlyFloating","Floatingdecay","Advancedfloatingdecay","SunkenRemains")) #fixes x-axis labels
...
plot_richness(physeq, x="DecompStage",color="DecompStage", measures=c("Shannon"),)+geom_boxplot(aes(x=DecompStage, y=value, color=DecompStage), alpha=0.05)+ylab("Shannon Diversity")+facet_wrap(~Location)
...
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = Phylum),colour="black", stat="identity")+xlab("DecompStage")+ylab("Relative Abundance (> 1%)")+theme(axis.title.x=element_blank())+ theme(axis.text.x = element_text(angle = 45, hjust = 1))+ scale_fill_manual(values=cbPalette)
...
```{r}
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = DecompStage),colour="black", stat="identity")+ facet_grid(~Phylum)+xlab("Decomp Stage")+ylab("Relative Abundance (> 2%)") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.title.x=element_blank())+facet_wrap(~Phylum)+geom_errorbar(aes(ymin=mean-se,ymax=mean+se))
...
```{r TreatmentPlot}
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = Family),colour="black", stat="identity")+xlab("DecompStage")+ylab("Relative Abundance (> 2%)")+theme(axis.title.x=element_blank())+ theme(axis.text.x = element_text(angle = 45, hjust = 1))#+ scale_fill_manual(values=cbPalette)
cdataplot#+  scale_x_discrete(labels=c("0 hrs", "24 hrs", "48 hrs","72 hrs"))
...
```{r}
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = DecompStage),colour="black", stat="identity")+ facet_grid(~Sample_Type)+xlab("Decomp Stage")+ylab("Relative Abundance (> 2%)") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.title.x=element_blank())+facet_wrap(~Family)+geom_errorbar(aes(ymin=mean-se,ymax=mean+se))
   #scale_x_discrete(labels=c("0 hrs", "24 hrs", "48 hrs","72 hrs"))+ theme(axis.text.x = element_text(angle = 45, hjust = 1))#+ scale_fill_manual(values=cbPalette)
...
df$Abundance=df$Abundance*100
Trtdata <- ddply(df, c("Genus", "DecompStage"), summarise,
                 N    = length(Abundance),
                 mean = mean(Abundance),
                 sd   = sd(Abundance),
                 se   = sd / sqrt(N)
...
```{r TreatmentPlot4}
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = Genus),colour="black", stat="identity")+xlab("DecompStage")+ylab("Relative Abundance (> 2%)") + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))+theme(axis.title.x=element_blank())+ theme(axis.text.x = element_text(angle = 45, hjust = 1))#+ scale_fill_manual(values=cbPalette)
cdataplot#+  scale_x_discrete(labels=c("0 hrs", "24 hrs", "48 hrs","72 hrs"))
#position = "fill"#+geom_errorbar(aes(ymin=mean-se,ymax=mean+se),color="black",width=1)+geom_line(size=1.5,linetype="dashed")+geom_point(size=6)+ylab(
...
cdataplot=ggplot(Trtdata, aes(x=DecompStage,y=mean))+geom_bar(aes(fill = DecompStage),colour="black", stat="identity")+ facet_grid(~Genus)+xlab("Decomp Stage")+ylab("Relative Abundance (> 2%)") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.title.x=element_blank())+facet_wrap(~Genus)+geom_errorbar(aes(ymin=mean-se,ymax=mean+se))
   #scale_x_discrete(labels=c("0 hrs", "24 hrs", "48 hrs","72 hrs"))+ theme(axis.text.x = element_text(angle = 45, hjust = 1))#+ scale_fill_manual(values=cbPalette)
cdataplot
Means=compare_means(Abundance ~ DecompStage, data = df, 
